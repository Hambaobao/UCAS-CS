...
void *tcp_client(void *arg)
{
	...
	FILE *rec_fp; 								
	if((rec_fp = fopen("record.txt", "w")) == NULL) 
	{ 
		printf("Open File Falied!\n"); 
		return NULL; 
	}

	u64 start_time = record[0].time.tv_sec * 1000 + record[0].time.tv_usec / 1000;
	for(int i = 0; i < record_index; i++)
	{
		u64 time = record[i].time.tv_sec * 1000 + record[i].time.tv_usec / 1000 - start_time;
		fprintf(rec_fp, "%lu\t%u\n", time, record[i].cwnd);				
		fflush(rec_fp);

		// printf("time: %lu, cwnd: %d\n", time, record[i].cwnd);
	}
