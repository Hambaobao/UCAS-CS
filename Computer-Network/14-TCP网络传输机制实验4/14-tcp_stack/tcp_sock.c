...
inline void tcp_set_con_state(struct tcp_sock *tsk, int state)
{
	log(DEBUG, IP_FMT":%hu switch con_state, from %s to %s.", \
			HOST_IP_FMT_STR(tsk->sk_sip), tsk->sk_sport, \
...
void init_tcp_stack()
{
	...
	pthread_mutex_init(&ip_send_lock, NULL);
	pthread_mutex_init(&snd_buf_lock, NULL);

	pthread_mutex_init(&retrans_lock, NULL);
	pthread_mutex_init(&timewait_lock, NULL);
...
struct tcp_sock *alloc_tcp_sock()
{
	...
	tsk->cwnd = TCP_DEFAULT_MSS;
	tsk->ssthresh = 0xffffffff;
...
void free_tcp_sock(struct tcp_sock *tsk)
{
	...
	if (tsk->ref_cnt == 0) 
	{
		printf("**** Free TCP Sock: ["IP_FMT":%hu<->"IP_FMT":%hu].",
				HOST_IP_FMT_STR(tsk->sk_sip), tsk->sk_sport,
...
struct tcp_sock *tcp_sock_lookup_established(u32 saddr, u32 daddr, u16 sport, u16 dport)
{
	...
	int tcp_hash = tcp_hash_function(saddr, daddr, sport, dport);
	struct list_head *list = &tcp_established_sock_table[tcp_hash];
...
struct tcp_sock *tcp_sock_lookup_listen(u32 saddr, u16 sport)
{
	...
	int tcp_hash = tcp_hash_function(0, 0, sport, 0);
	struct list_head *list = &tcp_listen_sock_table[tcp_hash];
...
int tcp_sock_connect(struct tcp_sock *tsk, struct sock_addr *skaddr)
{
	...
	if(tcp_sock_set_sport(tsk, tcp_get_port())) 
	{
		printf("**** TCP sock set sport failed\n");
		return -1;
	}
...
struct tcp_sock *tcp_sock_accept(struct tcp_sock *tsk)
{
	...
	struct tcp_sock *acc_tsk = tcp_sock_accept_dequeue(tsk);
	tcp_set_state(acc_tsk, TCP_ESTABLISHED);
	tsk->con_state = CON_OPEN;
...
static void tcp_sock_clear_listen_queue(struct tcp_sock *tsk)
{
	...
	{
		struct tcp_sock *entry = list_entry(tsk->listen_queue.next, struct tcp_sock, list);
		list_delete_entry(&entry->list);

		if (entry->state == TCP_SYN_RECV) 
		{
			entry->parent = NULL;
			tcp_unhash(entry);
			free_tcp_sock(entry);
...
{
	// printf("**** TCP socket read\n");

	while(ring_buffer_empty(tsk->rcv_buf))
	{
		// printf("**** ring buff empty, sleep wait recv\n");
		// sleep_on(tsk->wait_recv);
		// printf("**** waked up on wait_recv\n");
	}

	// printf("**** ring buff not empty\n");
...
{
	// printf("**** TCP socket write\n");

	int buf_len = min(len, 1514 - ETHER_HDR_SIZE - IP_BASE_HDR_SIZE - TCP_BASE_HDR_SIZE);
	int pkt_size = ETHER_HDR_SIZE + IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE + buf_len;

	char *packet = (char *)malloc(pkt_size);
	memset(packet, 0, ETHER_HDR_SIZE + IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE);
	memcpy(packet + ETHER_HDR_SIZE + IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE, buf, buf_len);

	usleep(20000);
	tcp_send_packet(tsk, packet, pkt_size);

	// printf("**** sleep wait send\n");
...
void record_cwnd(struct tcp_sock *tsk)
{
	...
	record[record_index].cwnd = tsk->cwnd;
	gettimeofday(&record[record_index].time, NULL);		
	record_index ++;
