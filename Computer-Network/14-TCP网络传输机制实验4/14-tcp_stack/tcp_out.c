...
void tcp_send_packet(struct tcp_sock *tsk, char *packet, int len) 
{
	...
	struct send_buf_entry *entry = (struct send_buf_entry *)malloc(sizeof(struct send_buf_entry));
	...
	// set tcp retrans
	tcp_set_retrans_timer(tsk, tsk->snd_nxt);
...
void tcp_send_control_packet(struct tcp_sock *tsk, u8 flags)
{
	...
	if (flags & (TCP_SYN | TCP_FIN))
	{
		...
		struct send_buf_entry *entry = (struct send_buf_entry *)malloc(sizeof(struct send_buf_entry));
		entry->packet = (char *)malloc(pkt_size);		
		entry->len = pkt_size;
		memcpy(entry->packet, packet, pkt_size);
