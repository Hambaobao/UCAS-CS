...
#define TCP_MAX_BACKLOG 128
	...
	struct tcp_retrans_timer retrans_timer;
	...
	struct dupack dup_ack;

	// tcp state, see enum tcp_state in tcp.h
	int state;

	// tcp congestion state, see enum tcp_con_state in tcp.h
	int con_state;
...
void tcp_set_con_state(struct tcp_sock *tsk, int state);
