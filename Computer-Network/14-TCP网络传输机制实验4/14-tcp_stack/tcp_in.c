...
static inline void move_ofo_packet_to_ring_buff(struct tcp_sock *tsk)
{
	...
	list_for_each_entry_safe(entry, q, &tsk->rcv_ofo_buf, list)
	{
		if(last_seq_flag)
		{
			...
			else if(entry->seq_num == global_seq_end)
			{
...
void increase_cwnd(struct tcp_sock *tsk)
{
	// slow start
...
void do_tcp_con_state_open(struct tcp_sock *tsk, struct tcp_cb *cb)
{
	struct dupack *dup_ack = &tsk->dup_ack;
	if (cb->ack == dup_ack->ack)
	{
		dup_ack->cnt ++;
		tcp_set_con_state(tsk, CON_DISORDER);
	}
	else
	{
		dup_ack->ack = cb->ack;
...
void do_tcp_con_state_disorder(struct tcp_sock *tsk, struct tcp_cb *cb)
{
	struct dupack *dup_ack = &tsk->dup_ack;
	if (cb->ack == dup_ack->ack)
	{
		...
		{
			tcp_set_con_state(tsk, CON_RECOVERY);			
			do_tcp_con_state_recovery(tsk, cb);
		}
	}
	else
	{
		tcp_set_con_state(tsk, CON_OPEN);
		dup_ack->ack = cb->ack;
		dup_ack->cnt = 0;
		increase_cwnd(tsk);
	}
...
void do_tcp_con_state_cwr(struct tcp_sock *tsk, struct tcp_cb *cb)
{
	return ;
}

void do_tcp_con_state_recovery(struct tcp_sock *tsk, struct tcp_cb *cb)
{
	tsk->cwnd >>= 1;
...
void do_tcp_con_state_loss(struct tcp_sock *tsk)
{
	tsk->ssthresh = tsk->cwnd >> 1;
	tsk->cwnd = TCP_DEFAULT_MSS;	
	tcp_set_con_state(tsk, CON_OPEN);
...
void handle_con_state(struct tcp_sock *tsk, struct tcp_cb *cb)
{
	switch (tsk->con_state)
	{
	case CON_OPEN:
		do_tcp_con_state_open(tsk, cb);
		break;
	
	case CON_DISORDER:
		do_tcp_con_state_disorder(tsk, cb);
		break;

	case CON_CWR:
		do_tcp_con_state_cwr(tsk, cb);
		break;
...
{	
	if(cb->flags & TCP_SYN) 
	{
		...
		csk->cwnd = TCP_DEFAULT_MSS;
		csk->ssthresh = 0xffffffff;
