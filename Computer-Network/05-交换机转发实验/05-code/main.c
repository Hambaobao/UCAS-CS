...
uint32_t handle_packte_count = 0;
void handle_packet(iface_info_t *iface, char *packet, int len)
{
	printf(">>>>>> The %d times handle packet\n", ++handle_packte_count);
	...
	iface_info_t *out_iface = lookup_port(eh->ether_dhost);
	pthread_mutex_unlock(&mac_port_map.lock);

	if(out_iface) {
		iface_send_packet(out_iface, packet, len);
		printf(">>>>>> Find port\n\n");
	}
	else {
		broadcast_packet(iface, packet, len);
		printf(">>>>>> Not find port\n\n");
	}
...
int main(int argc, const char **argv)
{
	...
	pthread_create(&t1, NULL, ustack_run, NULL);
