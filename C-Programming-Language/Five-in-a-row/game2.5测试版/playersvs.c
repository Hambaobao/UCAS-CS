...
//	for(int i=1;i<16;i++)
//		for(int j=1;j<16;j++)
//			bbq[i][j]=get_grades_black(i,j);
/*
	for(int i=1;i<16;i++)
		for(int j=1;j<16;j++)
			gradestable_black[i][j].score=get_grades_black(0,i,j)+get_grades_black(1,i,j)
							+get_grades_black(2,i,j)+get_grades_black(3,i,j);
	
	for(int i=1;i<16;i++)
		{
		for(int j=1;j<16;j++)
			printf("%d  ",gradestable_black[i][j].score);
		printf("\n");
		}
*/
