...
int get_my_grades(int x,int y){
	...
	&& (BlackScoreBoard[x][y].info[i].oppnum[0]>1 || BlackScoreBoard[x][y].info[i+4].oppnum[0]>1))			//黑棋眠4(不包含死4)
		return 3800;
...
int get_opp_grades(int x,int y){
	...
	&& (WhiteScoreBoard[x][y].info[i].oppnum[0]>1 || WhiteScoreBoard[x][y].info[i+4].linknum[0]>1))			//白棋眠3(真眠3)
		return 2300;
