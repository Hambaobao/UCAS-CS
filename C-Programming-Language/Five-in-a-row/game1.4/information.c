...
//记录黑棋棋盘上每一点的信息
	...
	for(int m=1;m<16;m++)	for(int n=1;n<16;n++)	for(int i=1;temp[m][n+i]==1;i++)	BlackScoreBoard[m][n].info[0].linknum++;		//记录——向连珠数
	for(int m=1;m<16;m++)	for(int n=1;n<16;n++)	for(int i=1;temp[m][n-i]==1;i++)	BlackScoreBoard[m][n].info[0].linknum++;
										
	for(int m=1;m<16;m++)	for(int n=1;n<16;n++)	for(int i=1;temp[m+i][n+i]==1;i++)	BlackScoreBoard[m][n].info[1].linknum++;		//记录 / 向连珠数
	for(int m=1;m<16;m++)	for(int n=1;n<16;n++)	for(int i=1;temp[m-i][n-i]==1;i++)	BlackScoreBoard[m][n].info[1].linknum++;

	for(int m=1;m<16;m++)	for(int n=1;n<16;n++)	for(int i=1;temp[m+i][n]==1;i++)	BlackScoreBoard[m][n].info[2].linknum++;		//记录 | 向连珠数
	for(int m=1;m<16;m++)	for(int n=1;n<16;n++)	for(int i=1;temp[m-i][n]==1;i++)	BlackScoreBoard[m][n].info[2].linknum++;

	for(int m=1;m<16;m++)	for(int n=1;n<16;n++)	for(int i=1;temp[m+i][n-i]==1;i++)	BlackScoreBoard[m][n].info[3].linknum++;		//记录 \ 向连珠数
	for(int m=1;m<16;m++)	for(int n=1;n<16;n++)	for(int i=1;temp[m-i][n+i]==1;i++)	BlackScoreBoard[m][n].info[3].linknum++;

	
	for(m=1;m<16;m++)	for(n=1;n<16;n++)	for(i=1;temp[m][n+i]==1;i++)	;	if(temp[m][n+i]==2)	BlackScoreBoard[m][n].info[0].oppnum++;		//记录——向对手棋子数
	for(m=1;m<16;m++)	for(n=1;n<16;n++)	for(i=1;temp[m][n-i]==1;i++)	;	if(temp[m][n-i]==2)	BlackScoreBoard[m][n].info[0].oppnum++;

	for(m=1;m<16;m++)	for(n=1;n<16;n++)	for(i=1;temp[m+i][n+i]==1;i++)	;	if(temp[m+i][n+i]==2)	BlackScoreBoard[m][n].info[1].oppnum++;		//记录/向对手棋子数
	for(m=1;m<16;m++)	for(n=1;n<16;n++)	for(i=1;temp[m-i][n-i]==1;i++)	;	if(temp[m-i][n-i]==2)	BlackScoreBoard[m][n].info[1].oppnum++;

	for(m=1;m<16;m++)	for(n=1;n<16;n++)	for(i=1;temp[m+i][n]==1;i++)	;	if(temp[m+i][n]==2)	BlackScoreBoard[m][n].info[2].oppnum++;		//记录 | 向对手棋子数
	for(m=1;m<16;m++)	for(n=1;n<16;n++)	for(i=1;temp[m-i][n]==1;i++)	;	if(temp[m-i][n]==2)	BlackScoreBoard[m][n].info[2].oppnum++;

	for(m=1;m<16;m++)	for(n=1;n<16;n++)	for(i=1;temp[m+i][n-i]==1;i++)	;	if(temp[m-i][n-i]==2)	BlackScoreBoard[m][n].info[3].oppnum++;		//记录\向对手棋子数
	for(m=1;m<16;m++)	for(n=1;n<16;n++)	for(i=1;temp[m-i][n+i]==1;i++)	;	if(temp[m+i][n+i]==2)	BlackScoreBoard[m][n].info[3].oppnum++;

//记录白棋棋盘上每一点的信息

	for(int m=1;m<16;m++)	for(int n=1;n<16;n++)	for(int i=1;temp[m][n+i]==2;i++)	WhiteScoreBoard[m][n].info[0].linknum++;		//记录——向连珠数
	for(int m=1;m<16;m++)	for(int n=1;n<16;n++)	for(int i=1;temp[m][n-i]==2;i++)	WhiteScoreBoard[m][n].info[0].linknum++;
										
	for(int m=1;m<16;m++)	for(int n=1;n<16;n++)	for(int i=1;temp[m+i][n+i]==2;i++)	WhiteScoreBoard[m][n].info[1].linknum++;		//记录 / 向连珠数
	for(int m=1;m<16;m++)	for(int n=1;n<16;n++)	for(int i=1;temp[m-i][n-i]==2;i++)	WhiteScoreBoard[m][n].info[1].linknum++;

	for(int m=1;m<16;m++)	for(int n=1;n<16;n++)	for(int i=1;temp[m+i][n]==2;i++)	WhiteScoreBoard[m][n].info[2].linknum++;		//记录 | 向连珠数
	for(int m=1;m<16;m++)	for(int n=1;n<16;n++)	for(int i=1;temp[m-i][n]==2;i++)	WhiteScoreBoard[m][n].info[2].linknum++;

	for(int m=1;m<16;m++)	for(int n=1;n<16;n++)	for(int i=1;temp[m+i][n-i]==2;i++)	WhiteScoreBoard[m][n].info[3].linknum++;		//记录 \ 向连珠数
	for(int m=1;m<16;m++)	for(int n=1;n<16;n++)	for(int i=1;temp[m-i][n+i]==2;i++)	WhiteScoreBoard[m][n].info[3].linknum++;

	
	for(m=1;m<16;m++)	for(n=1;n<16;n++)	for(i=1;temp[m][n+i]==2;i++)	;	if(temp[m][n+i]==1)	WhiteScoreBoard[m][n].info[0].oppnum++;		//记录——向对手棋子数
	for(m=1;m<16;m++)	for(n=1;n<16;n++)	for(i=1;temp[m][n-i]==2;i++)	;	if(temp[m][n-i]==1)	WhiteScoreBoard[m][n].info[0].oppnum++;

	for(m=1;m<16;m++)	for(n=1;n<16;n++)	for(i=1;temp[m+i][n+i]==2;i++)	;	if(temp[m+i][n+i]==1)	WhiteScoreBoard[m][n].info[1].oppnum++;		//记录/向对手棋子数
	for(m=1;m<16;m++)	for(n=1;n<16;n++)	for(i=1;temp[m-i][n-i]==2;i++)	;	if(temp[m-i][n-i]==1)	WhiteScoreBoard[m][n].info[1].oppnum++;

	for(m=1;m<16;m++)	for(n=1;n<16;n++)	for(i=1;temp[m+i][n]==2;i++)	;	if(temp[m+i][n]==1)	WhiteScoreBoard[m][n].info[2].oppnum++;		//记录 | 向对手棋子数
	for(m=1;m<16;m++)	for(n=1;n<16;n++)	for(i=1;temp[m-i][n]==2;i++)	;	if(temp[m-i][n]==1)	WhiteScoreBoard[m][n].info[2].oppnum++;

	for(m=1;m<16;m++)	for(n=1;n<16;n++)	for(i=1;temp[m+i][n-i]==2;i++)	;	if(temp[m-i][n-i]==1)	WhiteScoreBoard[m][n].info[3].oppnum++;		//记录\向对手棋子数
