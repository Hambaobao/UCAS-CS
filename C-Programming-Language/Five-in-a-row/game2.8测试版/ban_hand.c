...
//判断白棋“禁手”
int Ban_Hand_0(int x,int y){
	int Long_Connect_0(int ,int);
	int Three_Ban_0(int ,int);
	int Four_Ban_0(int ,int);
...
//长连禁手判断
int Long_Connect_0(int m,int n){
	int j=0;
	for(int i=0;i<4;i++)
		if(WhiteScoreBoard[m][n].info[i].linknum[0]+WhiteScoreBoard[m][n].info[i+4].linknum[0]>5)
			j++;
	return (j>0)?1:0;	
}

//33禁手判断
int Three_Ban_0(int m,int n){
	int three=0;
	for(int i=0;i<8;i++)
		if(WhiteScoreBoard[m][n].info[i].linknum[0]+WhiteScoreBoard[m][n].info[(i+4)%8].linknum[0]+WhiteScoreBoard[m][n].info[i].linknum[1]==3
			&& WhiteScoreBoard[m][n].info[i].nullnum[0]==1 && WhiteScoreBoard[m][n].info[i].nullnum[1]>0
			&& WhiteScoreBoard[m][n].info[(i+4)%8].nullnum[0]>0	)							// +010110+, +011010+
		three++;
	for(int i=0;i<4;i++)
		if(WhiteScoreBoard[m][n].info[i].linknum[0]+WhiteScoreBoard[m][n].info[(i+4)%8].linknum[0]==3
			&& WhiteScoreBoard[m][n].info[i].nullnum[0]>0 && WhiteScoreBoard[m][n].info[(i+4)%8].nullnum[0]>0	)	// +01110+
		three++;
	for(int i=0;i<4;i++)
		if(WhiteScoreBoard[m][n].info[i].linknum[0]+WhiteScoreBoard[m][n].info[(i+4)%8].linknum[0]==3
			&& WhiteScoreBoard[m][n].info[i].nullnum[0]==1 && WhiteScoreBoard[m][n].info[(i+4)%8].nullnum[0]==1
			&& WhiteScoreBoard[m][n].info[i].oppnum[0]>0 && WhiteScoreBoard[m][n].info[(i+4)%8].oppnum[0]>0		)	//排除 2011102 （假活三）
		three--;
...
//44禁手判断
int Four_Ban_0(int m,int n){
	int four=0;
	for(int i=0;i<8;i++)
		if(WhiteScoreBoard[m][n].info[i].linknum[0]+WhiteScoreBoard[m][n].info[(i+4)%8].linknum[0]==4 
			&& WhiteScoreBoard[m][n].info[i].nullnum[0]>0	)		// +01111+, +11110+
		four++;
	for(int i=0;i<8;i++)
		if(WhiteScoreBoard[m][n].info[i].linknum[0]+WhiteScoreBoard[m][n].info[(i+4)%8].linknum[0]+WhiteScoreBoard[m][n].info[i].linknum[1]==4
			&& WhiteScoreBoard[m][n].info[i].nullnum[0]==1	)		//+10111+, +11011+, +11101+
		four++;
	
	for(int i=0;i<4;i++)
		if(WhiteScoreBoard[m][n].info[i].linknum[0]+WhiteScoreBoard[m][n].info[(i+4)%8].linknum[0]==4 
			&& WhiteScoreBoard[m][n].info[i].nullnum[0]>0 && WhiteScoreBoard[m][n].info[(i+4)%8].nullnum[0]>0 )	//排除假双四（活四）+011110+
		four--;
	return (four>1)?1:0;
}
