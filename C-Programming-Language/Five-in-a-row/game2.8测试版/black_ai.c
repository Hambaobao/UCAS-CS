...
struct point AI_Black_0(int n){
	int candidates_black(long long int pmax[8],struct point a[250]);//用于选出待选点，存到MAX数组
	struct point zl;
	...
	l=candidates_black(pmax,BlackGradesMax);	
...
//产生黑棋的待选落子位置
int candidates_black(long long int pmax[8],struct point BlackGradesMax[250]){
