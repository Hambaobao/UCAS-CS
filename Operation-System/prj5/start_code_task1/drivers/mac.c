...
int UnRecv[PNUM];   // Denote Recved Or Not
int Recv_NO(desc_t *Recv_Desc)
{
    int i;
    for(i = 0;i <PNUM; i++)
    {
        if(Recv_Desc[i].tdes0 & 0x80000000)
            ;           // Haven't Recv Package
        else if(UnRecv[i])
        {
            UnRecv[i] = 0;
            return i;   // Package i has Recved           
        }       
    }
    ...
    {
        if(UnRecv[i])
            return -1;  // There is still Package Haven't been Recved
    }

    return -2;  // No more Package
}
...
uint32_t do_net_recv(uint32_t rd, uint32_t rd_phy, uint32_t daddr)
{
    ...
    while(Recv_Desc[0].tdes0 & 0x80000000)        // Polling
    {
        vt100_move_cursor(1, 1);
        printk("Receiving Package");
    }

    for(i = 0; i < PNUM; i++)                   // Print Information
    {
        if(!(Recv_Desc[i].tdes0 & 0x80000000))
        {
            n++;
            ...
            for(j = 0;j < PSIZE; j++)
                printk("%x ",((uint32_t *)(daddr))[PSIZE * i + j]);
        }       
    }
