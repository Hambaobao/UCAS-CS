...
#define L1_VPAGE_NUMB 1024
#define L2_VPAGE_NUMB 1024

//L1 Entry Flags // R = Reserved
// 31                              12 11 9  8   7   6  5             1    0
// +--------------20-----------------+----+---+---+---+----+-----+----+---+
// |     L2  Page Table NUMB         |                           |IMM| V |
// +---------------------------------+----+---+---+---+---+-----+---------+

typedef struct L1_pte {
    uint32_t l1_entry;
} L1_pte_t;

#define CVDG_Flags0 ((PTE_C << 3) | (PTE_D << 2) | (PTE_V << 1) | PTE_G) & 0x3f
#define CVDG_Flags1 ((PTE_C << 3) | (PTE_D << 2) | (PTE_V << 1)) & 0x3f
