...
{
    // malloc virtual address from 0x0000 0000 - 0x0001 0000 (sum of 524288 * 4KB = 2GB)
    // BUut the lowest virtual address from 0x0000 0000 - 0x0001 0000 is reserved for kernel
    pageframe_t *frame;
    int vaddr;
    int j;
    ...
    // paging 8MB first & On-Demond paging
    // 524288 * 4K = 2GB
    for(vaddr = 0x00000000, j = 0; j < 2048; j++, vaddr += PAGE_SIZE)
    {
        frame = palloc();
        pcb[3].page_table[vaddr >> 12].ptentry = (frame->paddr & 0xfffff000) | (CDVG_Flags << 6) | (PTEV << 5);       
    }

    for(; j < NUMB_VPAGE; j++, vaddr += PAGE_SIZE)
    {
        pcb[3].page_table[vaddr >> 12].ptentry = 0x0;       
    }
}
...
//    uint32_t vaddr0,vaddr1;
    ...
    sdread((uint8_t *)(badvaddr & 0xffffe000),SD_addr0,PAGE_SIZE);
    sdread((uint8_t *)((badvaddr & 0xfffff000) | 0x1000),SD_addr1,PAGE_SIZE);   
}
